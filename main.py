...
def main():
    ...
    parser.add_argument('--task_id', default=1, help='BABI task ID', type=int)
    ...
    parser.add_argument('--gate_penalty', '-gpenalty',
                        help="Penalty on inter memory cell interactions.",
                        default=0.0, type=float)
    parser.add_argument('--max_epochs', '-mepochs',
                        help='Maximum number of epochs',
                        default=200, type=int)
    parser.add_argument('--bow_encoding', '-bow_enc', default=False,
                        action='store_true',
                        help='Whether to use BoW encoding for sentences.')
    parser.add_argument('--init_lr', help="Initial learning rate",
                        default=1e-2, type=float)
    parser.add_argument('--learning_rate_decay_epochs', '-lr_decay',
                        default=25, help='Learning rate decay rate', type=int)
    ...
    if args.bow_encoding:
        model_name += 'bow_'
    if args.learning_rate_decay_epochs != 25:
        model_name += 'lrd_' + str(args.learning_rate_decay_epochs) + '_'
    if args.init_lr != 1e-2:
        model_name += 'init_lr_' + str(args.init_lr) + '_'
    if args.gate_penalty:
        model_name += "gpenalty_" + str(args.gate_penalty) + "_"
    ...
    # Now carry out the experiment
    do_experiment(data_dir=args.data_dir, model_dir=model_path,
