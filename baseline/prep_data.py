...
def get_max_sentence_lengths(train_iter):
    max_sentence_length = 0
    max_query_length = 0
    for train_batch in train_iter:
        x_train, _ = train_batch
        stories, queries, answers = x_train
        sentence_length = list(stories.size())[2]
        if sentence_length > max_sentence_length:
            max_sentence_length = sentence_length

        query_length = list(queries.size())[1]
        if query_length > max_query_length:
            max_query_length = query_length

    return (max_sentence_length, max_query_length)

# %% Get the iterators for train, validation, and test
def get_iter(data_dir, task_id=1, batch_size=32, tenK=True, joint=False, memory_size=None):
    if memory_size is None:
        if task_id == 3:
            ...
            memory_size = min(70, calc_memory_size(data_dir, task_id))
    # print ("Max story size:", memory_size)
    train_iter, valid_iter, test_iter = datasets.BABI20.iters(batch_size=batch_size, root=data_dir, task=task_id, joint=joint, memory_size=memory_size, tenK=tenK, shuffle=True)
    if joint:
        test_iter = {}
        for task in range(1, 21):
            _, _, task_test_iter = datasets.BABI20.iters(batch_size=batch_size, root=data_dir, task=task, joint=joint, memory_size=memory_size, tenK=tenK, shuffle=True)
            test_iter[task] = task_test_iter
...
if __name__=='__main__':
    ...
    get_data(DATA_DIR)
    train_iter, valid_iter, test_iter = get_iter(DATA_DIR, tenK=True, joint=True)
