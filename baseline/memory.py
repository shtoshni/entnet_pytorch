...
class DynamicMemory(nn.Module):
    def __init__(self, hidden_size=100, memory_slots=20, keys=None, activation=None):
        ...
        self.gate_bias = Parameter(nn.init.normal_(torch.empty(memory_slots), mean=0, std=0.1))
    ...
    def calc_gate(self, batch_sent, memory):
        ...
        memory: M x B x H
        """
        batch_size = list(batch_sent.size())[0]
        memory_term = torch.sum(memory * batch_sent, 2)  # M x B
        key_term = torch.mm(self.keys, torch.transpose(batch_sent, 1, 0))  # M x B
        return torch.sigmoid(memory_term + key_term + torch.unsqueeze(self.gate_bias, 1))  # M x B
    ...
    def calc_candidate(self, batch_sent, memory):
        ...
        memory_term = self.U(memory_resh)  # (M x B) x H
    ...
    def read_story(self, stories, mask):
        ...
        """
        batch_size, hidden_size = list(stories[0].size())
